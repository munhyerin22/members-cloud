...
{
  "app": {
    "message": "팀원의 정보를 저장하고 조회하는 애플리케이션"
  }
}
```
...
### 📎 Presigned URL (과제 제출용)

> 🚨 유효기간 7일 설정 완료

```
https://project-cloud-prod-profile-files.s3.ap-northeast-2.amazonaws.com/uploads/5b75f039-ccff-403b-a133-cc54b8694d68_%EC%9D%B4%EC%84%B8%EB%8F%8C%20%EC%99%84%EC%A0%84%EC%B2%B43.png?[서명파라미터]
```

> ⚠️ **만료 시간**: 과제 제출일 기준 7일 후  
> ※ 최신 Presigned URL은 `GET /api/members/1/profile-image` 호출로 재발급 가능
...
- API 요청 시: `INFO` 레벨 → `[API - LOG] {메서드명} 메서드 호출`
- 예외 발생 시: `ERROR` 레벨 → `[API - ERROR] 발생 원인: {스택트레이스}`